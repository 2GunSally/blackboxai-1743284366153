...
class KrogerChecker:
    ...
    def _rotate_proxy(self):
        """Get a fresh proxy from FreeProxy with retries and testing"""
        max_retries = 3
                ...
                if self.proxy:
                    ...
                    try:
                        ...
                        if test_response.status_code == 200:
                            logger.info(f"Proxy working: {self.proxy}")
                            return
    ...
    def _get_auth_token(self, email: str, password: str) -> str:
        """Attempt to get auth token from Kroger login with retries"""
        max_retries = 2
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"Rate limited, rotating proxy...")
                    self._rotate_proxy()
                    continue
                return None
...
def main():
    print("Kroger Credential Checker (Proxy Fixed Version)")
