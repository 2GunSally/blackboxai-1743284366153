...
def connect_imap(server, email, password):
    for attempt in range(3):  # Try up to 3 times
        try:
            ...
            try:
                logger.info(f"Attempting standard login...")
                result, response = imap.login(email, password)
                if result == "OK":
                    logger.info("Login successful")
                    return imap
            except imaplib.IMAP4.error:
                logger.info("Standard login failed, trying alternative method...")
                result, response = imap._simple_command('AUTHENTICATE', 'LOGIN', email, password)
                if result == "OK":
